...
import { Truck, User, FileText, Shield, CheckCircle, AlertCircle, Hash } from 'lucide-react';
...
const VehicleRegistration: React.FC = () => {
  ...
  const approvedCompanies = state.companies?.filter(c => c.status === 'approved') || [];

  const validateForm = (): boolean => {
    const newErrors: Record<string, string> = {};

    if (!formData.companyId) newErrors.companyId = 'Please select a company';
    if (!formData.registrationNumber.trim()) newErrors.registrationNumber = 'Vehicle registration number is required';
    ...
    if (!formData.driverMobile.trim()) newErrors.driverMobile = 'Driver mobile number is required';
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Vehicle Registration Submitted!</h2>
            <p className="text-gray-600 mb-6">
              Your vehicle has been registered and is pending verification. You will receive updates within 24-48 hours.
            </p>
            ...
            <div className="bg-blue-50 rounded-lg p-6 mb-6">
              ...
              <div className="space-y-3 text-left">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-blue-800">Vehicle Registration Verification</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                  <span className="text-blue-800">Insurance Verification (Optional but recommended)</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                  <span className="text-blue-800">Driver License Cross-Check</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                  <span className="text-blue-800">VCODE Activation & Shipment Assignment</span>
                </div>
              </div>
            </div>
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Vehicle Registration</h2>
            <p className="text-gray-600">Register vehicles for your approved logistics company</p>
          </div>
      ...
      {approvedCompanies.length === 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-yellow-800">No Approved Companies</h3>
              <p className="text-yellow-700">You need to have an approved company registration before registering vehicles.</p>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                {errors.weightCapacity && (
                  <p className="text-red-500 text-sm mt-1">{errors.weightCapacity}</p>
                )}
              </div>
              ...
              <div>
                ...
                {errors.volumeCapacity && (
                  <p className="text-red-500 text-sm mt-1">{errors.volumeCapacity}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-purple-200 p-6">
            ...
            <div className="space-y-3 text-purple-800">
              <p className="flex items-center space-x-2">
                ...
                <span>Each approved vehicle receives a unique VCODE for automatic shipment assignment</span>
              </p>
              <p className="flex items-center space-x-2">
                <CheckCircle className="h-4 w-4" />
                <span>VCODE is generated after successful verification (24-48 hours)</span>
              </p>
              <p className="flex items-center space-x-2">
                <Truck className="h-4 w-4" />
                <span>Shipments are automatically assigned to available registered vehicles</span>
              </p>
            </div>
          </div>
