...
import { MapPin, Navigation, Clock, Package, Phone, User, CheckCircle, AlertCircle } from 'lucide-react';
import { useApp } from '../context/AppContext';

const LiveTracking: React.FC = () => {
  const { state, dispatch } = useApp();
  const [selectedShipment, setSelectedShipment] = useState(state.selectedShipment || state.shipments[0]?.id || '');

  const currentShipment = state.shipments.find(s => s.id === selectedShipment);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <p className="text-gray-600">Real-time tracking of all active shipments with AI-optimized routes.</p>
            ...
            <div className="space-y-3">
              {state.shipments.map((shipment) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 mb-2">{shipment.from} → {shipment.to}</p>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-600">{currentShipment.currentLocation}</span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-lg h-64 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-lg font-medium text-gray-900">Live Map View</p>
                    <p className="text-sm text-gray-600">Real-time GPS tracking with AI-optimized routes</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{currentShipment.id}</p>
                        <p className="text-sm text-gray-600">Customer: {currentShipment.customer}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{currentShipment.from} → {currentShipment.to}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-700">Status</p>
                        <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(currentShipment.status)}`}>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{currentShipment.estimatedDelivery}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{currentShipment.driver}</p>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{currentShipment.vehicle}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm text-gray-600">{currentShipment.driverPhone}</span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">{currentShipment.progress}% Complete</p>
                      </div>
                    </div>
                  </div>
                </div>
